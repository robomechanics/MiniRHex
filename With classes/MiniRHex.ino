...
leg_params leg0 = { IDS[0], 0,{1, 1, -1, -1,  1, 1 },    0, zeros[0],  false, false, false };
leg_params leg1 = { IDS[1], 0,{1, 1, -1, -1,  1, 1 },    0, zeros[1],  false, false, false };
leg_params leg2 = { IDS[2], 0,{1, 1, -1, -1,  1, 1 },    0, zeros[2],  false, false, false };
leg_params leg3 = { IDS[3], 0,{1, 1,  1, -1, -1, 1 },    0, zeros[3],  true,  false, false };
leg_params leg4 = { IDS[4], 0,{1, 1,  1, -1, -1, 1 },    0, zeros[4],  true,  false, false };
leg_params leg5 = { IDS[5], 0,{1, 1,  1, -1, -1, 1 },    0, zeros[5],  true,  false, false };
leg_params legs[6] = { leg1, leg2, leg3, leg4, leg5, leg6 };
    ...
    if (!legs[i].deadzone){
      ...
      if (legs[i].gait == STAND){ //standing or sitting
        if (legs[i].right_side){
          legs[i].params.desired_theta = Theta_to_ThetaR(legs[i].params.desired_theta);
        }
        else{
          legs[i].params.desired_theta = legs[i].params.desired_theta;
        }
        legs[i].set_vel_act(actual_theta - legs[i].params.zero)
		; //zero out leg thetas, accounts for small servo irregularities
        control_signal = legs[i].get_control_signal();  //pd_controller(actual_theta, desired_theta, actual_vel, 0, kp_hold, kd_hold); 
      }
        ...
        actual_theta = actual_theta - legs[i].params.
		zero;
